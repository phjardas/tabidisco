...
import { Player, Library, PiAdapter, Song, Play } from './api';
...
export class TabidiscoImpl implements Tabidisco {
  ...
  constructor(
    @inject(Types.Bus) public readonly bus: Bus,
    @inject(Types.Player) private readonly player: Player,
    @inject(Types.Library) private readonly library: Library,
    @inject(Types.PiAdapter) private readonly pi: PiAdapter
  ) {
